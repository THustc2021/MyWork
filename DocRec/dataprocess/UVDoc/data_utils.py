...
def crop_image_tight(img, wtexture, grid2D):
    """
    ...
    wtexture = wtexture[miny - s : maxy + s, minx - s : maxx + s, :]
    ...
    wtexture = wtexture[cy1:-cy2, cx1:-cx2, :]
    top = miny - s + cy1
    bot = size[0] - maxy - s + cy2
    left = minx - s + cx1
    right = size[1] - maxx - s + cx2
    return img, wtexture, top, bot, left, right
...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def crop_tight(self, img_RGB, wimg_RGB, grid2D):
        # The incoming grid2D array is expressed in pixel coordinates (resolution of img_RGB before crop/resize)
        size = img_RGB.shape
        img, wimg, top, bot, left, right = crop_image_tight(img_RGB, wimg_RGB, grid2D)
        img = cv2.resize(img, self.img_size)
        img = img.transpose(2, 0, 1)
        img = torch.from_numpy(img).float()
        wimg = cv2.resize(wimg, self.img_size)
