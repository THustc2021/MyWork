...
class SpatialAttn(nn.Module):
    def __init__(self, in_channels, reduction_ratio=8):
        ...
        self.attn_conv = nn.Sequential(
            nn.Conv2d(in_channels, in_channels // reduction_ratio, kernel_size=5, stride=1, padding=2),
...
class IllDecoder(nn.Module):
    ...
    def forward(self, x0, x1, x2, x3, x):
        ...
        x = self.olayers1(self.se1(x))
        x = self.olayers2(self.se2(x) + x3)
        x = self.olayers3(self.se3(x) + self.spe3(x2))
        x = self.olayers4(self.se4(x) + self.spe4(x1))
