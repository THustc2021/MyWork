...
class SpatialAttnUpsample(nn.Module):
    def __init__(self, in_channels, out_channels, reduction_ratio=8):
        super(SpatialAttnUpsample, self).__init__()

        self.attn_conv = nn.Sequential(
            nn.ConvTranspose2d(2 * in_channels, in_channels // reduction_ratio, kernel_size=3, stride=2, padding=1, output_padding=1),
...
class IllDecoder(nn.Module):
    ...
    def __init__(self, norm_fn="batch", acf=nn.ReLU(inplace=True),
        ...
        self.deep_conv1 = self._make_layer(channel_lists[1], stride=-2)

        self.se2 = SEModule(channel_lists[1])
        self.attn2 = SpatialAttnUpsample(channel_lists[1], channel_lists[2])

        self.se3 = SEModule(channel_lists[2])
        self.attn3 = SpatialAttnUpsample(channel_lists[2], channel_lists[3])

        self.se4 = SEModule(channel_lists[3])
        self.attn4 = SpatialAttnUpsample(channel_lists[3], channel_lists[4])
    ...
    def forward(self, x0, x1, x2, x3, x):
        ...
        x = self.deep_conv1(self.se1(x))
        x = self.attn2(self.se2(x), x3)
        x = self.attn3(self.se3(x), x2)
        x = self.attn4(self.se4(x), x1)
        # 暂时用不到
