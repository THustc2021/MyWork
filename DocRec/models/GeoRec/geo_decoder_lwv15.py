...
class GeoDecoder(nn.Module):
    def __init__(self,  norm_fn, acf):
        ...
        self.attn_hl = AttentionBlock(256, 128, 16, 128, norm, acf, type=1)
        self.attn_tc = AttentionBlock(128, 32, 16, 32, norm, acf, type=0)
        self.coords_embedding = nn.Conv2d(2, 32, 1, 1, 0)
        self.upsample = Upsampler(32, 2, norm, acf, up_scale=8)
    ...
    def forward(self, x0, x1, x2, x3, f, x):
        ...
        trans_feature = self.attn_hl(hq_feature, f)
        # 坐标映射
        b, c, h, w = trans_feature.shape
        coords = gen_coords(h, w)[None].to(trans_feature)
        # pn_embed = get_positional_embeddings(coords[:, 0], coords[:, 1], 32 // 2).repeat(b, 1, 1, 1)
        pn_embed = self.coords_embedding(coords).repeat(b, 1, 1, 1)
        coords_feature = self.attn_tc(pn_embed, trans_feature)
        # 上采样
