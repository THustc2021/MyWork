...
class GeoDecoder(nn.Module):
    def __init__(self,  norm_fn, acf):
        ...
        self.attn2 = AttentionBlock(128, 192, 64, 128, norm=norm, acf=acf, type=0)
        self.add_and_norm2 = Add_and_Norm(128, acf)

        self.attn0 = AttentionBlock(128, 128, 32, 128, norm, acf)
        self.add_and_norm5 =  Add_and_Norm(128, acf)

        self.attn = AttentionBlock(128, 32, 32, 32, norm, acf, type=1)
    ...
    def forward(self, x0, x1, x2, x3, f, x):
        ...
        f4 = self.add_and_norm5(self.attn0(f2, f2), f2)
