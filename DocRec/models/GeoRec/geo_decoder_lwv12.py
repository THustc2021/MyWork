...
class GeoDecoder(nn.Module):
    def __init__(self,  norm_fn, acf):
        ...
        self.attn_hq_with_coords0 = AttentionBlock(32, 32, 16, 32, norm, acf, type=1)
        #
        # self.spattn1 = SpatialAttn(64, downsample_rate=8)
        self.attn_hq_with_coords1 = AttentionBlock(32, 64, 16, 64, norm, acf, type=1)
        #
        # self.spattn2 = SpatialAttn(128, downsample_rate=4)
        self.attn_hq_with_coords2 = AttentionBlock(64, 128, 16, 128, norm, acf, type=1)
        #
        # self.spattn3 = SpatialAttn(192, downsample_rate=2)
        self.attn_hq_with_coords3 = AttentionBlock(128, 192, 16, 192, norm, acf, type=1)

        self.upsample1 = Upsampler(192, 64, norm, acf, up_scale=4)
        self.upsample2 = Upsampler(64, 16, norm, acf, up_scale=4)
    ...
    def forward(self, x0, x1, x2, x3, f, x):
        ...
        # 上采样
        grid = self.upsample1(coords_feature)
        grid = self.upsample2(grid)
