...
from models.GeoRec.geo_decoder_lwv2 import GeoRec
...
def train(idx, dataloader, model, writer, geo_loss_f, total_loss_f, save_result=None, cuda=True):
    ...
    for i, data in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        with torch.amp.autocast(device_type='cuda' if cuda else 'cpu', dtype=torch.float16):
            ...
            geo_out = model(img)
            pred = F.grid_sample(img, geo_out.permute(0, 2, 3, 1), align_corners=False)

            # 2 部分
            loss = geo_loss_f(geo_out, grid)
    ...
    if save_result != None and (epoch+1) % 30 == 0:
...
def evaluate(dataloader, model, valid_metric_f, save_result=None, cuda=True):
    ...
    for i, data in enumerate(dataloader):
        ...
        with torch.no_grad():
            pred = model(img)
            pred = F.grid_sample(img, pred.permute(0, 2, 3, 1), align_corners=False)
        # 计算loss
...
if __name__ == '__main__':
    ...
    train_dataset = RealDataset(img_transform=train_transform, grid_path="/home/xtanghao/THPycharm/dataset/DocUnet Benchmark/gt/grid")
    ...
    model.load_model("/home/xtanghao/DocRec/results/GeoRecCPTR/03-27_01-17/model_4.pth")
    ...
    geo_loss_f = GeoLoss(w_bnd=0.01, w_local=20)
