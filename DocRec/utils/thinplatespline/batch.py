...
def K_matrix(X, Y):
    ...
    Y : (N, m, 2) torch.tensor of m points in 2 dimensions.
...
class TPS_coeffs(torch.nn.Module):
    ...
    Y : torch tensor (N, K, 2), eg. a UV map.
...
class TPS(torch.nn.Module):
    ...
    ----------
    X : torch tensor (N, K, 2), eg. projected points.
    Y : torch tensor (N, K, 2), for example, UV maps.
    ...
    def __init__(self, size: tuple = (256, 256), device="cuda"):
        super().__init__()
        h, w = size
        self.size = size
        self.device = device
...
if __name__ == '__main__':
    ...
    X = torch.randn(64, 32, 2).cuda()
    Y = torch.randn(64, 32, 2).cuda()
    tps = TPS()
    grid = tps(X, Y)
    print(grid.shape)