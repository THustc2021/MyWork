...
class InterfaceEC():
    def __init__(self, pop_size, m, api_endpoint, api_key, llm_model, debug_mode, interface_eval, **kwargs):
        # -------------------- RZ: use local LLM --------------------
        ...
        self.evol = Evolution(api_endpoint, api_key, llm_model, debug_mode, **kwargs)
        self.m = m
    ...
    def population_generation_seed(self,seeds):
        ...
        for seed in seeds:  # 测试每个种子的性能
            ...
            try:
                ...
                e = datetime.now()
                print(f"evaluate a time: {(e-s).seconds/60} m")
