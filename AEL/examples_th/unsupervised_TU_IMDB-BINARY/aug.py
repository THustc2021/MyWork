...
class TUDataset_aug(InMemoryDataset):
    ...
    def get(self, idx):
        ...
        if self.aug == "eva":
            ...
            if n == 0:
                if self.eva.__name__ == "drop_nodes":
                    data_aug = self.eva(deepcopy(data))
                else:
                   data_aug = drop_nodes(deepcopy(data))
            elif n == 1:
                if self.eva.__name__ == "permute_edges":
                    data_aug = self.eva(deepcopy(data))
                else:
                    data_aug = permute_edges(deepcopy(data))
            elif n == 2:
                if self.eva.__name__ == "subgraph":
                    data_aug = self.eva(deepcopy(data))
                else:
                    data_aug = subgraph(deepcopy(data))
            elif n == 3:
                if self.eva.__name__ == "mask_nodes":
                    data_aug = self.eva(deepcopy(data))
                else:
