...
class TUDataset_aug(InMemoryDataset):
    ...
    def download(self):
        ...
        path ="/home/xtanghao/THPycharm/AEL_main/examples/unsupervised_TU/NCI1.zip"
    ...
    def get(self, idx):
        ...
        if self.aug == 'dnodes':
            data_aug = drop_nodes(deepcopy(data),self.eva)
            ...
            n = np.random.choice(5, 1, p=self.aug_P)[0] # 以随机概率选择增强方法
            if n == 0:
               data_aug = drop_nodes(deepcopy(data),self.eva)
            elif n == 1:
               data_aug = permute_edges(deepcopy(data))
...
def drop_nodes0(data):
    ...
    return data
def drop_nodes(data,eva):
    data.edge_index,data.x = eva(data.edge_index,data.x)
    return data
