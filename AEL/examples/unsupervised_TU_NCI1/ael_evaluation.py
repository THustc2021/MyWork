    ...
    for epoch in range(1, epochs + 1):
        ...
        for data in dataloader:
            ...
            if args.aug == 'dnodes' or args.aug == 'subgraph' or args.aug == 'random2' or args.aug == 'random3' or args.aug == 'random4' or args.aug == 'minmax':
                ...
                data_aug.batch = data.batch[idx_not_missing]    # 更新增强数据的批索引
                ...
                data_aug.edge_index = torch.tensor(edge_idx).transpose_(0, 1)   # 将更新后的索引转换回张量形式

            data_aug = data_aug.to(device)
            x_aug = model(data_aug.x, data_aug.edge_index, data_aug.batch, data_aug.num_graphs) # 使用模型对增强后的数据进行前向传播，得到嵌入表示

            loss = model.loss_cal(x, x_aug) # 计算原始数据和增强数据之间的对比损失
            print(loss)
            loss_all += loss.item() * data.num_graphs   # 当前批次的损失累积到总损失中，乘以图的数量
            loss.backward()
            optimizer.step()
