...
class MultiAgentPolicyManager(BasePolicy):
    ...
    def process_fn(  # type: ignore
        ...
        for agent, policy in self.policies.items():
            ...
            ### 处理state
            if hasattr(tmp_batch.policy, "hidden_state"):
                tmp_batch.policy.hidden_state = tmp_batch.policy.hidden_state[agent].state
            #
        ...
        **kwargs: Any,
        ...
        for agent_id, policy in self.policies.items():
            ...
            if len(agent_index) == 0:
                ...
                # 这里我们保存state，不直接清零
                results.append((False, np.array([-1]), Batch(), Batch(), Batch() if state is None else state[agent_id]))
                continue
