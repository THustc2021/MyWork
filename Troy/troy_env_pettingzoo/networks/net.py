...
class Actor(nn.Module):
        ...
        for i in range(len(self.max_actions)):
            if i == 3 or i == 4:
                self.cls_head.append(
                    nn.Linear(self.output_dim, action_shape[0]*2*(self.restrict_range+1)) # 前后两个方向
                )
            else:
                self.cls_head.append(
    ...
    def forward(
        ...
        if self.is_training and random.random() < self.random_state:  # 每个actor都有一定概率执行随机操作
            # 为每个最后一个维度创建一个随机变量
            commands = []
            for command_idx in range(len(self.max_actions)):
                if command_idx == 3:
                    command = np.random.rand(*mask.shape[:-1], 2*(self.restrict_range + 1))
                    command = command.argmax(axis=-1)
                    command = command - self.restrict_range + obs.obs_self[:, :, -4]
                    command = command.clip(0, self.max_actions[command_idx])
                elif command_idx == 4:
                    command = np.random.rand(*mask.shape[:-1], 2*(self.restrict_range + 1))
                    command = command.argmax(axis=-1)
                    command = command - self.restrict_range + obs.obs_self[:, :, -3]
                    command = command.clip(0, self.max_actions[command_idx])
                else:
                    command = np.random.rand(*mask.shape[:-1], self.max_actions[command_idx]+1)
                    command = command.argmax(axis=-1)
                commands.append(command)
            logits = np.stack(commands, axis=-1)
            # 动作掩码
            logits[~mask.astype(bool)] = 0
            return torch.from_numpy(logits), state
        else:
            # 预处理
            ...
            for mi in range(len(self.cls_head)):
                ...
                if self.exploration_noise > 0:
                    t = t + torch.randn_like(t) * self.exploration_noise
                t = torch.argmax(t, dim=2)
                # 对坐标特殊处理
                if mi == 3:
                    t = t - self.restrict_range + torch.from_numpy(obs.obs_self[:, :, -4]).to(logits)
                    t = t.clamp(0, self.max_actions[mi])
                elif mi == 4:
                    t = t - self.restrict_range + torch.from_numpy(obs.obs_self[:, :, -3]).to(logits)
                    t = t.clamp(0, self.max_actions[mi])
                ts.append(t[..., None])
            # 拼接结果
            logits = torch.cat(ts, dim=2).to(logits)
            # 后续处理（应用mask）
            logits[torch.from_numpy(~mask.astype(bool))] = 0
