...
from networks.net import Actor, Critic, PreProcessNet
from networks.random_policy import RandomPolicy
...
buffer_size = 102400
logdir = "results"
task = "Troy_2agents_with_dist_gamma_0.85"
train_name = "autoset"

# 改为每一步都有可能进行随机操作
# troy_act_random = False
# greece_act_random = False
...
def get_agents(load_policy=False, exploration_noise=0.) :
    ...
    actor = Actor(ppprocess_net, net, action_space.shape or action_space.n,
                  max_actions=action_space.high[0], restrict_range=MAX_RANGE-1, device=device,
                  exploration_noise=exploration_noise, is_training=True).to(device)
    ...
    agent_troy = DDPGPolicy(
        ...
        lr_scheduler=torch.optim.lr_scheduler.CosineAnnealingLR(critic_optim, T_max=1000, eta_min=1e-5)
    )
    ...
    if load_policy:
        ...
        model_troy_save_path = os.path.join(logdir, task, f"{train_name}_pth", "troy_policy_5.pth")
        model_greek_save_path = os.path.join(logdir, task, f"{train_name}_pth", "greek_policy_5.pth")
...
if __name__ == '__main__':
    ...
    # train_envs = ts.env.DummyVectorEnv([lambda: get_env() for _ in range(2)])
